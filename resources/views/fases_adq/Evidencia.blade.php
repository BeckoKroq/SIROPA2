...
@section('content')
                ...
                <table class="table">
                    ...
                    <tbody>
                    	<tr>
                            ...
                            <td>{!!link_to_route('img.index', $title='', $parameters = $adquisicion, $attributes=['class'=>'far fa-images fa-2x'])!!}</td>
                    	</tr>
                        <tr>
                            <td>Cheque</td>
                            <?php $list_pdf= DB::table('evidencia')->where('obras_publicas_id', $adquisicion)->where('numero',2)->pluck('path_file_pdf')?>
                            <?php $pdf3 = ''?>
                        ...
                        <tr>
                            <td>Estimaciones</td>
                            <?php $list_pdf= DB::table('evidencia')->where('obras_publicas_id', $adquisicion)->where('numero',3)->pluck('path_file_pdf')?>
                            <?php $pdf1 = ''?>
                            @if(strlen($list_pdf)>6)
                                <?php $list_pdf= DB::table('evidencia')->where('obras_publicas_id', $adquisicion)->where('numero',3)->pluck('path_file_pdf')?>
                                    @for($i=0; $i<=count($list_pdf); $i++)
                        ...
                        <tr>
                            <td>Generadores</td>
                            <?php $list_pdf= DB::table('evidencia')->where('obras_publicas_id', $adquisicion)->where('numero',4)->pluck('path_file_pdf')?>
                            <?php $pdf2 = ''?>
                            @if(strlen($list_pdf)>6)
                                <?php $list_pdf= DB::table('evidencia')->where('obras_publicas_id', $adquisicion)->where('numero',4)->pluck('path_file_pdf')?>
                                    @for($i=0; $i<=count($list_pdf); $i++)
    ...
    <div class="modal-wrapper" id="popup_f1">
        <div class="popup_f1-contenedor">
            ...
            <a class="popup_f1-cerrar" href="{{ route('fasetres.edit',$adquisicion) }}">X</a>
        </div>
    </div>
    <div class="modal-wrapper" id="popup19_f1">
        <div class="popup19_f1-contenedor">
            ...
            <a class="popup19_f1-cerrar" href="{{ route('fasetres.edit',$adquisicion) }}">X</a>
        </div>
    </div>
    <div class="modal-wrapper" id="popup20_f1">
        <div class="popup20_f1-contenedor">
            ...
            <a class="popup20_f1-cerrar" href="{{ route('fasetres.edit',$adquisicion) }}">X</a>
        </div>
    </div>
@endsection
